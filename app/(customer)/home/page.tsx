...
import { Bike, Clock, Shield, Star, ArrowRight, Zap, Heart, TrendingUp, Loader2 } from "lucide-react";
...
export default function LandingPage() {
  ...
  const [stats, setStats] = useState<Stats>({ totalBookings: 0, totalMotors: 0, averageRating: 4.9 });
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch stats (optimized single endpoint)
      ...
      // Fetch motors
      const motorsResponse = await fetch("/api/motors");
      const motorsData = await motorsResponse.json();

      if (motorsResponse.ok) {
        // Get top 3 featured motors (available motors)
        const availableMotors = motorsData.motors.filter((m: Motor) => m.status === "Available");
        const featured = availableMotors.slice(0, 3);
        setFeaturedMotors(featured);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        {/* Hero Section */}
        <section className="relative overflow-hidden px-4 py-28 md:py-44" aria-label="Hero section">
                ...
                <motion.div variants={fadeIn} className="mt-12 flex flex-wrap items-center gap-6 border-t border-border pt-8">
                  <div>
                    <div className="flex items-baseline gap-1">
                      <span className="font-serif text-3xl font-semibold text-foreground">{loading ? "-" : stats.totalBookings}+</span>
                  ...
                  <div>
                    <div className="font-serif text-3xl font-semibold text-foreground">{loading ? "-" : stats.totalMotors}+</div>
                    <p className="text-sm text-muted-foreground">Quality Motors</p>
                  </div>
                  <div className="h-8 w-px bg-border" />
                  <div>
                    <div className="flex items-baseline gap-1">
                      <span className="font-serif text-3xl font-semibold text-foreground">{stats.averageRating}</span>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15, margin: "-60px" }} variants={stagger} className="mb-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {loading ? (
                ...
                <div className="col-span-full text-center py-12">
                  <p className="text-muted-foreground">No motors available at the moment</p>
                </div>
              ) : (
                featuredMotors.map((motor, idx) => (
                  <motion.div key={motor.id} variants={fadeInUp}>
                    {idx === 1 && featuredMotors.length >= 3 ? (
                      <FeaturedCard>
                        <Card variant="gradient" className="h-full border-0 shadow-none">
                          <div className="relative h-56 overflow-hidden rounded-t-2xl bg-muted">
                            {motor.image ? (
                              <img
                                src={motor.image}
                                alt={`${motor.brand} ${motor.model} ${motor.year} - Premium ${motor.type} motorcycle available for rent at ${motor.pricePerDay.toLocaleString("id-ID")} IDR per day`}
                          ...
                          <CardContent>
                            <div className="flex items-end justify-between">
                              <div>
                                <p className="font-serif text-3xl font-semibold text-foreground">Rp {motor.pricePerDay.toLocaleString("id-ID")}</p>
                      ...
                      <Card variant="elevated" className="group h-full overflow-hidden">
                        <div className="relative h-56 overflow-hidden bg-muted">
                          {motor.image ? (
                            <img
                              src={motor.image}
                              alt={`${motor.brand} ${motor.model} ${motor.year} - ${motor.type} motorcycle for rent in Indonesia - Starting from ${motor.pricePerDay.toLocaleString("id-ID")} IDR daily`}
                        ...
                        <CardContent>
                          <div className="flex items-end justify-between">
                            <div>
                              <p className="font-serif text-2xl font-semibold text-foreground">Rp {motor.pricePerDay.toLocaleString("id-ID")}</p>
                              <p className="text-sm text-muted-foreground">per day</p>
                            </div>
      ...
      {/* Footer */}
      <footer className="border-t border-border bg-muted px-4 py-16" role="contentinfo">
        <div className="container mx-auto max-w-6xl">
